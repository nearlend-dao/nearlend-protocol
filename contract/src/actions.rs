...
impl Contract {
    ...
    pub fn internal_force_close_nft(
        &mut self,
        account_id: &AccountId,
        account: &mut Account,
        prices: &Prices,
        ...
        let mut borrowed_sum = BigDecimal::zero();
        let mut collateral_sum = BigDecimal::zero();
        let mut nft_collateral_sum = BigDecimal::zero();

        let mut affected_farms = vec![];

        // Transfer all FT to assets reserved
        for (token_id, shares) in liquidation_account.supplied.drain() {
            let mut asset = self.internal_unwrap_asset(&token_id);
            let amount = asset.supplied.shares_to_amount(shares, false);
            asset.reserved += amount;
        ...
        // Transfer all NFT to liquidator
        for (contract_nft_token_id, account_nft_asset) in liquidation_account.nft_supplied.drain() {
            let asset = self.internal_unwrap_asset(&account_nft_asset.nft_contract_id);

            // Add NFT to liquidator account
            account.internal_set_nft_asset(&contract_nft_token_id, account_nft_asset.clone());

            let balance = 10u128.pow(24);
            nft_collateral_sum = nft_collateral_sum
                + BigDecimal::from_balance_price(
                    balance,
            ...
            // Transfer owner NFT to liquidator account
            self.internal_set_nft_asset(
                &account_nft_asset.nft_contract_id,
                account_id.clone(),
                account_nft_asset.nft_token_id,
                asset,
            );
        }
        ...
        assert!(
            borrowed_sum > total_collateral_sum,
            "Total borrowed sum {} is not greater than total collateral sum {}",
            borrowed_sum,
